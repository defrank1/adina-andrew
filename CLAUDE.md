...
- `rsvp.html` — **Metro Rive intro → two-stage landing (matched cut, then settle into a brand page).** The nav's RSVP link points here (`/rsvp`). After the guest password (`october17` / `siteUnlocked`), the hand-drawn DC Metro intro plays: `js/rive-intro.js` owns a `requestAnimationFrame` loop that scrubs the linear timeline (`Timeline 1` in `assets/rive/metro-intro.riv`) at a **15fps stepped cadence**, using `Fit.Contain` so the 16:9 scene is never cropped — the tunnel-green matte (`#183a2c`) fills the letterbox on tall/wide/mobile viewports. At `DURATION_S` (22.14s, card fully expanded) it holds ~0.5s, then **hard-cuts (no fade)** to `#invitation-endstate` in its Rive-matched state (green matte + flat `#dfdeda` 16:9 field + the invitation card `images/invitation/invitation.svg`, no chrome). ~150ms later (`SETTLE_DELAY_MS`) the page **settles** over `SETTLE_MS` (450ms, via `body.intro-complete` + `var(--settle-ms)`): matte/field fade out to the standard textured cream (dark mode via the normal mechanism — grain is intentionally NOT in the .riv, it arrives here), nav + footer fade in (present-but-hidden during the intro, not `display:none`), the card gains the `.registry-illustration` emboss (cream fill kept in dark mode), and a **replay icon** fades in bottom-left (placeholder DC-flag asset, `data-light`/`data-dark` swap; clears `intro-seen` + reloads — one playback pipeline; hidden under `prefers-reduced-motion`). Return visits, reduced-motion, and missing-asset bails land settled **instantly** (no green, no transition). Plays once per session (`intro-seen`). `<body class="page-rsvp">`. The **sequenced RSVP form is the next step** — it will grow out of this settled invitation page. (See decisions.md, July 6, 2026.)
