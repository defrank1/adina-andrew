...
- **Light mode:** `assets/textures/combined-light.png` — warm white `#F1EDEA` with four grain layers pre-composited at soft-light in Figma
- **Dark mode:** `assets/textures/combined-dark.png` — dark green `#122a20` with two grain layers pre-composited at soft-light in Figma
...
├── assets/                 (ALL media — the image subfolders live here directly; no assets/ superfolder)
│   ├── favicon/            (favicon.png)
...
- **Our Story page** uses `.story-timeline` container (600px max-width) with `.story-moment` blocks. Each section has narrative prose in `.story-prose` (Sentient body, 480px max-width, opacity 0.85, multiple `<p>` children allowed) followed by an optional photo group, then a `.story-caption-label` footer. Photo groups can be: `.story-photo-single` (380px max, mobile: 320px), `.story-photo-pair` (flex row, mobile: stacked column), or `.story-carousel` (multi-photo with arrows + dots — see Carousel below). `.story-divider` hairlines between sections. `.story-closing` as final text after the last divider. Web-optimized JPGs prefixed `web-` in `assets/our-story/`. The page does not use `.page-intro` — the Meeting prose is the first content. Old `.story-caption` class still defined but no longer used in HTML (kept available for potential future reuse).
- **Story carousels** (`.story-carousel` family) — used for multi-photo sections on the Our Story page. Container is max-width 480px, centered. Track uses native `scroll-snap-type: x mandatory` over a flex row (`.story-carousel-track` with `tabindex="0"` for keyboard); each slide is `flex: 0 0 100%` with `scroll-snap-align: center` — **one photo per frame**, flex-centered and `object-fit: contain`, so a photo narrower than the 480px frame sits centered with blank (letterbox) space on the sides rather than showing its neighbors (an earlier dimmed-filmstrip approach was dropped in favor of this). Prev/next arrows (`.story-carousel-arrow-prev` / `-next`) mirror the `.btn-normal` debossed aesthetic at ~38px circular: same `feTurbulence` `::before` grain, same `--shadow-raised`/`--shadow-lifted`/`--shadow-pressed` token transitions, same dark mode overrides. PP Watch chevron glyph. Desktop: arrows positioned `left: -50px` / `right: -50px` (outside photo). Mobile (≤900px): arrows overlay inside photo edges (`left: 8px` / `right: 8px`) with semi-transparent backdrop and `backdrop-filter: blur(2px)`. Dots (`.story-carousel-dot` inside `.story-carousel-dots`) are 6×6px circles, generated by JS from slide count — `dark-green` active, `rgba(26, 58, 46, 0.25)` inactive; dark mode equivalents using `--color-soft-white`. Disabled state: `.disabled` class with `opacity: 0.35` + `aria-disabled="true"`, click handler skips. Pattern uses `data-carousel` on the container and `data-carousel-dots` on the dots placeholder — JS auto-discovers all `[data-carousel]` elements. **Navigation is authoritative**: arrows/dots call `setActive(index)` to set the current photo directly (dots, caption, arrow-disabled state) rather than inferring it from scroll position — this fixes carousels whose last photos are narrow and share the final scroll frame. Free-scroll still infers the centered photo by position, with an end/start clamp. Logic lives in `includes/carousel.js`, loaded only on `our-story.html` via a separate `<script>` tag after `site-init.js`.
- **Story photo lightbox** — clicking **any** Our Story photo (carousel slide, `.story-photo-single`, or `.story-photo-pair`) opens it enlarged over a dimmed full-page scrim (`.story-lightbox`, one shared overlay appended to `<body>`, `z-index: 2000`, dark green-black `rgba(10,22,16,0.94)`), with the photo `object-fit: contain` (max 94vw / 82vh) and its caption beneath in PP Watch caps. Closes on scrim click, the `×` button, or Escape; `body` scroll locks while open and restores on close. Photos carry a `zoom-in` cursor. Built + wired in `includes/carousel.js` (`ensureLightbox` / `makePhotoZoomable`).
